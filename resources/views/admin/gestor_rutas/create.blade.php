...
<form action="{{ route('rutas.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <textarea name="recomendaciones" rows="3" class="form-control rounded-3 py-2"
                              placeholder="Ej. Llevar repelente, calzado adecuado, suficiente agua...">{{ old('recomendaciones') }}</textarea>
                </div>
    ...
    {{-- ══════════════════════════════════════
         BLOQUE 2: Resumen (duración, distancia, fechas)
    ══════════════════════════════════════ --}}
    <div class="ea-card p-0 overflow-hidden mb-4">
        <div class="p-4 border-bottom" style="border-color: var(--ea-line) !important; background: rgba(255,255,255,.25);">
            <div class="fw-semibold" style="color: var(--ea-text);">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="text" name="duracion_estimada" value="{{ old('duracion_estimada') }}"
                           class="form-control rounded-3 py-2"
                           placeholder="Ej. 1 día, 6 horas" maxlength="50">
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="number" name="distancia_km" value="{{ old('distancia_km') }}"
                           ...
                           step="0.01" min="0" placeholder="Ej. 45.5">
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="date" name="fecha_inicio_operacion"
                           value="{{ old('fecha_inicio_operacion') }}"
                           class="form-control rounded-3 py-2">
                    @error('fecha_inicio_operacion')<div class="small text-danger mt-1">{{ $message }}</div>@enderror
                </div>
                ...
                <div class="col-12 col-md-6">
                    ...
                    <input type="date" name="fecha_fin_operacion"
                           value="{{ old('fecha_fin_operacion') }}"
                           class="form-control rounded-3 py-2">
                    @error('fecha_fin_operacion')<div class="small text-danger mt-1">{{ $message }}</div>@enderror
                </div>

                <div class="col-12">
                    <div class="p-3 rounded-3" style="background:#f0f7f4; border-left: 3px solid var(--ea-green);">
                        <div class="small text-muted">
                            <i class="bi bi-info-circle me-1"></i>
                            Las fechas definen en qué período del año estará disponible esta ruta.
                            Por ejemplo: solo del 1 de febrero al 30 de abril.
                            Si no se indican, la ruta estará disponible todo el año.
                        </div>
                    </div>
                </div>
    ...
    <div class="ea-card p-0 overflow-hidden mb-4">
        ...
        <div class="p-4">
            ...
            <label class="form-label fw-bold">Paradas de la ruta <span class="text-danger">*</span></label>
            <div class="d-flex align-items-center gap-3 mb-4">
                ...
                <div class="p-2 px-3 rounded-3 fw-semibold small"
                    ...
                    <span id="contador-paradas">0</span> parada(s)
                </div>

                <button type="button" class="btn ea-btn-green rounded-3" onclick="agregarParada()">
                    <i class="bi bi-plus-lg me-1"></i> Agregar parada
                </button>
    ...
    <div class="ea-card p-0 overflow-hidden mb-4">
        ...
        <div class="p-4">
            <p class="text-muted small mb-3">
                ...
                numerados en el orden en que fueron agregados.
            </p>
    ...
    <div class="ea-card p-0 overflow-hidden mb-4">
        ...
        <div class="p-4">
            ...
            <input type="file" id="fotosInput" name="fotos[]"
                   accept="image/png,image/jpeg" class="d-none" multiple>
...
function recalcularNumeros() {
    ...
    bloques.forEach((bloque, index) => {
        ...
        if (etiqueta) etiqueta.textContent = numero === 1
            ? 'Parada 1 — Lugar de encuentro'
            : `Parada ${numero}`;
    });
...
async function actualizarMapa() {
    ...
    if (ordenes.length === 1) {
        ...
        mapaRuta.setCenter({ lat: punto.lat, lng: punto.lng });
        mapaRuta.setZoom(13);
        return;
    }
    ...
    directionsService.route({
        ...
        if (estado === google.maps.DirectionsStatus.OK) {
            ...
            console.warn('Directions API falló:', estado);
...
async function dibujarLineaRecta(ordenes) {
    ...
    ordenes.forEach(orden => {
        const punto = puntosRuta[orden];
        coords.push({ lat: punto.lat, lng: punto.lng });
        agregarMarcador(orden, punto, AdvancedMarkerElement);
    });

    lineaRuta = new google.maps.Polyline({
        path: coords, strokeColor: '#1F6B4B',
        strokeOpacity: 0.85, strokeWeight: 4, map: mapaRuta,
    });
